...
# 같은 deck JSON과 HTML renderer로 HyperFrames 영상 렌더 + QA
...
영상은 별도 React renderer를 만들지 않습니다. 정상 HTML 덱과 동일한 `deck.json`과
`build_deck()` 결과를 사용하고, 각 슬라이드를 독립 HyperFrames sub-composition으로 나눕니다.
검증된 semantic motion은 seek 가능한 paused GSAP cue로 표현하며, 영상 전용 콘텐츠나 레이아웃을
다시 작성하지 않습니다. 저장소의 Pretendard WOFF2를 기본 영상 폰트로 내장하므로 GitHub runner의
시스템 폰트에 의존하지 않습니다.

`render_video.py`는 HTML 생성, HyperFrames project 분리, cue/manifest 계약 검증,
`hyperframes check`, semantic cue 기준 PNG snapshot, MP4 렌더, system `ffprobe` 기반
codec/해상도/fps/duration 검증과 QA JSON 출력을 한 번에 수행합니다.

## GitHub Actions 영상 렌더

영상 파이프라인 변경은 `.github/workflows/video-check.yml`에서 모든 브랜치와 PR을 대상으로 계약 테스트와 실제 HyperFrames composition 검사를 수행합니다.
전체 MP4 렌더는 비용과 시간을 아끼기 위해 자동으로 실행하지 않고, `Render HyperFrames video` workflow를 수동 실행합니다.
기본 입력은 저장소 안의 deck JSON 경로, artifact 이름과 worker 수이며, 성공하면 MP4, `*.qa.json`, 생성된 composition과 핵심 프레임 PNG를 7일 동안 artifact로 보관합니다.
...
cd video && npm install && cd ..
python tools/hyperframes_project.py examples/modular-showcase.json --out-dir dist/video-ci --font assets/fonts/PretendardVariable.woff2
