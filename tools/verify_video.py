#!/usr/bin/env python3
"""Verify HyperFrames timing, sampled PNG frames, and an encoded MP4."""
...
def probe_video(video: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise VideoPipelineError("ffprobe command not found") from exc
...
def verify_video_artifacts(
    ...
    *,
    ...
    if len(video_streams) != 1:
        raise VideoPipelineError(f"expected exactly one video stream, got {len(video_streams)}")
    ...
    if abs(actual_duration - expected_duration) > tolerance:
        raise VideoPipelineError(
            f"video duration mismatch: expected {expected_duration:.3f}s, got {actual_duration:.3f}s"
        )
...
def main(argv=None) -> int:
    ...
    try:
        timing = read_video_timing(args.timing)
        if args.video:
            ...
            report = {"timing": validate_video_timing(timing)}
            if args.frames_dir:
                summary = report["timing"]
                report["sampledFrames"] = verify_frame_directory(
                    args.frames_dir,
                    sample_frames(timing),
                    width=summary["width"],
                    height=summary["height"],
                )
