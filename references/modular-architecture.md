...
Themes define design grammar such as corner and border treatment without owning
layout geometry or deck colors. Art direction lives in independent typography,
palette and data-visualization packs under `styles/`. Layouts own slots and geometry.
Information blocks own schemas and renderers. Visual blocks ingest reviewed raster
assets. Motion modules constrain supported intents and target block types. The core
owns fixed-stage scaling, active slide and phase state, accessibility and presenter
UI. Presets are full ordinary deck specs, so all of their choices can be edited.

## Selection

Layout search matches intent/tags, density and theme eligibility. Art-direction
search separately scores theme recommendation, domain, audience, tone, topic/tag
overlap, density and typography language support. Both use explicit numeric rules,
not embeddings or a remote semantic-search service. The planner records the chosen
style packs, reasons and top alternatives. Explicit style IDs bypass ranking.

The planner filters incompatible layouts using backtracking slot assignment; pinned
...
reached. Typography, main colors and chart colors resolve once at deck level and are
emitted as body data attributes plus CSS variables. Every transition is reversible.
A focus ring is inside its real target.
...
included. No editable chart/text PPTX export is promised. Typography packs define
fallback stacks but do not bundle extra font binaries; system-font builds can vary
between computers unless an authorized WOFF2 is explicitly embedded. Browser QA is
a geometry/behavior gate, not a full WCAG
